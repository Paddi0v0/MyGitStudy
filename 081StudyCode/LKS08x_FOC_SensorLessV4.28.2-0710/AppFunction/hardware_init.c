...
void CMP_init(void)
{
	...
	CMP_InitStruct.FIL_CLK10_DIV2 = 0x1;        /*0x0:1分频，0x1:2分频,0x2:4分频,0x3:8分频*/
...
void DAC_init(void)
{
    ...
    SYS_AFE_DAC = 100;  //83                    /* 1.2* X /4096/0.005 = 5A, 其中0.005为母线采样电阻*/ 
}
...
void ADC0_init(void)
{
    ...
    ADC_InitStructure.FirSeg_Ch = 10;                        /* 第一段共采样6个通道 */
...
//    GPIO_Init(GPIO3, &GPIO_InitStruct);
   ...
   /*LED1 & LED2  P0.6 & P0.7*/
    ...
    GPIO_SetBits(GPIO0,GPIO_Pin_6);   //P06输出高电平
...
/*  LED3  P0.3*/
    ...
    GPIO_Init(GPIO0, &GPIO_InitStruct);
    GPIO_ResetBits(GPIO0,GPIO_Pin_3);   //P0.3输出低电平
...
void MCPWM_init(void)
{
    ...
    MCPWM_InitStructure.FAIL0_INT_EN = DISABLE;               /* FAIL0事件 中断使能或关闭 */   //刹车急停事件
