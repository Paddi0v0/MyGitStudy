...
{	
	...
	switch(t_eMotorStatus)
	{
		...
		{			
			...
			HAL_DRV0_MCPWM_RegUpdate(tS_pMotorFoc->pMdToHd);		/* 加载PWM模块占空比值 */
		...
		{			
			...
			//if(E_PWM_ON == tS_pMotorFoc->ePWMStatusAck)
			{
				...
				tS_pMotorFoc->m_pHandle->Svpwm(t_bObjId,&tS_pFocElememt->mStatVolAB);
					...
					SVPWM_2Shunt_En1(t_bObjId,&tS_pFocElememt->mStatVolAB);
					#elif (EPWM0_CURRENT_SAMPLE_TYPE == CURRENT_SAMPLE_1SHUNT)
					SVPWM_1Shunt(t_bObjId,&tS_pFocElememt->mStatVolAB);
					#else
