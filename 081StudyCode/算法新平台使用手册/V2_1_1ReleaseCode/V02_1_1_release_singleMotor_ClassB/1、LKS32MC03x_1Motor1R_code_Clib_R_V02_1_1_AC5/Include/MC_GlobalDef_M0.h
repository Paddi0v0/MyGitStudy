...
#define DEADTIME_NS_M0                       ((u16)1200)		// 死区时间
#define DEADTIME_M0                          (u16)(((unsigned long long)PWM_MCLK_M0 * (unsigned long long)DEADTIME_NS_M0)/1000uL)

#if (CURRENT_SAMPLE_TYPE_M0 == CURRENT_SAMPLE_1SHUNT)
#define ADC_COV_TIME_M0						(1250)   			// 预留的ADC转换时间，单位：1250ns

#define SAMP_STABLE_TIME_SHUNT_M0			(1200)	 			// 1000--1800单电阻采样，信号稳定时间设置| 单位 nS
#else
#define ADC_COV_TIME_M0						(2500)   			// 预留的ADC转换时间，单位：2500ns
#define SAMP_STABLE_TIME_SHUNT_M0			(1500)	 			// 两三电阻采样，信号稳定时间设置| 单位 nS
#endif

#define SAMP_PROPAGATE_TIME_M0				(200)   /* 传输延时时间，单位：500--200ns*/

#define VF_DEADZONE_COMP_FACTOR_M0			(950)					// Q15格式: 0~32767 	
																//			0----- 根据死区时间自动计算
...
#define SEEK_POSITION_STATUS_M0				(TRUE)			// 初始位置检测状态 TRUE 为使能, FALSE 为不使能

/* ----------------------顺逆风检测是否是能---------------------------------------------------------------- */
#define DIR_CHECK_STATUS_M0					(TRUE)			// 顺逆风检测状态 TRUE 为使能, FALSE 为不使能
