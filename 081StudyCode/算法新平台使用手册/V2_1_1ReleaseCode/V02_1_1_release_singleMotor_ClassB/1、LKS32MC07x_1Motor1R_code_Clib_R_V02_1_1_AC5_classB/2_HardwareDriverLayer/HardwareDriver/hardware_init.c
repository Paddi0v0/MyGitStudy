...
void ADC0_init(void)
{
    ...
    ADC_InitStructure.S2         = 6;                   // 第二段常规采样次数 4
    ...
    ADC_InitStructure.S1         = 8;                   // 第一段常规采样次数
...
{
	u32 wSysRstSrc1=0;
	wSysRstSrc1=SYS_GetRstSource();		//读取复位源标志位 BIT0：POR复位 
	if(wSysRstSrc1 & BIT0)				// BIT3：看门狗复位
	{
...
{
	u32 wSysRstSrc2=0;
	wSysRstSrc2=SYS_GetRstSource();		//读取复位源标志位 BIT0：POR复位  
	if(wSysRstSrc2&BIT3)				//  BIT3：看门狗复位
	{
		return TRUE;					//查询复位状态寄存器
	}
	else 
