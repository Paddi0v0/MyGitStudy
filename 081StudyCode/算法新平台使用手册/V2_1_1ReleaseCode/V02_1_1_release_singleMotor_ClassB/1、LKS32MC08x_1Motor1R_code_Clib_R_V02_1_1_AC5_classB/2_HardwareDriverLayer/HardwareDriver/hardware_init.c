...
void Hardware_init(void)
{
	...
	NVIC_SetPriority(UART0_IRQn, 0);  /* 设置UART0中断优先级为2 | 共0，1，2，3四级中断优先级，0为最高*/
...
void PGA_init(void)
{
	...
	SYS_AFE_REG0 = OPA_mode | (OPA_mode<<2) | (OPA_mode<<4) | (OPA_mode<<6); /* 4个OPA增益设置 */
...
void DAC_init(void)
{
    ...
    SYS_AFE_DAC = wDACCmp;                     /* 1.2*512/4096/0.005 = 30A, 其中0.05为母线采样电阻*/ 
}
...
void ADC0_init(void)
{
    ...
    ADC_InitStructure.FirSeg_Ch = 12;                       /* 第一段共采样6个通道 */
...
void UTimer_init(void)
{
    ...
    TIM_InitStruct.Timer_IRQEna = Timer_IRQEna_CH0|Timer_IRQEna_Zero;       /* 开启Timer模块过0中断和捕获中断 */
...
void MCPWM_init(void)
{
    ...
    MCPWM_InitStructure.MCPWM_PERIOD = prd;            /* 计数周期设置 */
    MCPWM_InitStructure.TriggerPoint0 = (u16)(40 - prd); /* MCPWM_TMR0 ADC触发事件T0 设置 */
    MCPWM_InitStructure.TriggerPoint1 = (u16)(prd - 1);/* MCPWM_TMR1 ADC触发事件T1 设置 */
...
#elif (PRE_DRIVER_POLARITY == P_HIGH__N_HIGH)                    /* CHxP 高有效， CHxN高电平有效 */
...
void seekPosADCsetting(void)
{
    ...
    MCPWM_TMR0 = PWM_PERIOD_M0;   /*ADC采样时刻0设置为下桥关断中心点 */
...
/*************************************
存在上电过程,进行CLASSB 初始化校验
*************************************/
bool getIfPwrONReset(void)
{
	u32 wSysRstSrc1=0;
	wSysRstSrc1=SYS_GetRstSource();		//读取复位源标志位 BIT0：LPOR复位 
	if(wSysRstSrc1 & BIT1)				// BIT1：HPOR复位  BIT3：看门狗复位
	{
	  return TRUE;						//查询复位状态寄存器
	}
	else 
	{
	 return FALSE;	
...
存在看门狗复位记录
*************************************/
bool getIfWDReset(void)
{
	u32 wSysRstSrc2=0;
	wSysRstSrc2=SYS_GetRstSource();		//读取复位源标志位 BIT0：LPOR复位  
	if(wSysRstSrc2&BIT3)				//BIT1：HPOR复位  BIT3：看门狗复位
	{
		return TRUE;					//查询复位状态寄存器
	}
	else 
