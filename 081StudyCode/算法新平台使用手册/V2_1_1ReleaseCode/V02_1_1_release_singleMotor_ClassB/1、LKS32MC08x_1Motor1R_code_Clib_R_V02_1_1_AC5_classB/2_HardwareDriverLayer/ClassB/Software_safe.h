...
#define   CRC_SECTOR_SIZE          0x200              //从SAFTEY_MEMORY_ADDR开始CRC_SECTOR_SIZE个字节存储CRC数据
#define   SAFTEY_MEMORY_ADDR       0xf000             //MarchC+检测结果和flash校验golden结果存储地址，此地址一定要大于FLASH_SIZE
#define   ROM_INSERT_ERR_ADDR      0xa000             //ROM自检插入故障时的flash地址，此地址一定要小于FLASH_SIZE且是空闲没有被代码占用的

#define   CRC_RESULT_BUFFER        0x20001E00         //flash crc自检中间结果存储RAM地址

#undef    RAM_SIZE
#undef    FLASH_SIZE
#define   RAM_SIZE                 0x1FFE              //要检测的RAM空间，LKS08x 最大设置0x1FFE,LKS05x 最大设置0x9FE,LKS03x 最大设置0xFFE,
#define   RAM_ITER                 (RAM_SIZE >> 1)    //(RAM_SIZE>>1)

#define   FLASH_FULL_SIZE          0x10000            //Flash全部大小
#define   FLASH_SIZE               0xFE00             //flash CRC校验的空间范围，CRC每次检测32位数据
#define   FLASH_ITER               ((FLASH_SIZE - CRC_SECTOR_SIZE)  >> 2)  //FLASH_SIZE/32

//#define   FLASH_SIZE2              0x200             //flash CRC校验的空间范围，CRC每次检测32位数据
//#define   FLASH_ITER2              (FLASH_SIZE2 >> 2)  //FLASH_SIZE/32

#define   TEST_PART1_NUMS          32                 //REG_ITER+APSR
...
//typedef struct//volatile union 
//{   
//	 u32 crc_golden;            /* FLASH校验值 */
//	 u32 MarchC_result;         /* MarchC+自检结果 */
//	 u8 buf[(512 - sizeof(Software_SafteyStruct_o))];
// 
...
typedef struct
{
	 ...
	 volatile u16 rom_index;              /* REG & RAM & ROM 自检计数 */
	 volatile u16 rom_cnt;                /* REG & RAM & ROM 自检计数 */
	 volatile u16 rom_err_numb;           /* REG & RAM & ROM 自检出故障的位置 */
	 volatile u16 ram_index;              /* REG & RAM & ROM 自检计数 */
	 volatile u16 ram_err_numb;           /* REG & RAM & ROM 自检出故障的位置 */
	 volatile u16 reg_index;              /* REG & RAM & ROM 自检计数 */
	 volatile u16 reg_err_numb;           /* REG & RAM & ROM 自检出故障的位置 */
