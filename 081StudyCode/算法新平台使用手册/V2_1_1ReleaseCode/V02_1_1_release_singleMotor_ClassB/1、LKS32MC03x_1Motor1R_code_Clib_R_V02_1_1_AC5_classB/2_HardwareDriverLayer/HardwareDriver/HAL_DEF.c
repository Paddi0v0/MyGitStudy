...
void ADC_1Shunt_NormalModeCFG(void)
{
    ...
    ADC_CHN1 = ADC_BUS_VOL_CHANNEL | (BEMF_CH_A << 4) | (BEMF_CH_B << 8) | (BEMF_CH_C << 12);         // DATA4|DATA5|DATA6|DATA7
    ADC_CHN2 = INTERNAL_VSS_CHANNEL | (INTERNAL_REF_CHANNEL << 4) ;
}
...
{
    ADC_CHN0 = ADC0_CURRETN_A_CHANNEL | (ADC0_CURRETN_B_CHANNEL << 4) | (BEMF_CH_A << 8) | (BEMF_CH_B << 12) ;
    ADC_CHN1 = ADC_BUS_VOL_CHANNEL | (BEMF_CH_C << 4) | (ADC0_SPIN_CHANNEL << 8);
	ADC_CHN2 = INTERNAL_VSS_CHANNEL | (INTERNAL_REF_CHANNEL << 4) ;
...
{
    ADC_CHN0 = ADC0_CURRETN_A_CHANNEL | (ADC0_CURRETN_B_CHANNEL << 8);
    ADC_CHN1 = ADC0_CURRETN_A_CHANNEL | (ADC0_CURRETN_B_CHANNEL << 8);
    ADC_CHN2 = ADC_BUS_VOL_CHANNEL | (M0_ADC_BUS_CURR_CH << 8);  
}
...
void ChangeADChanelCFGM0(STR_PubDatMDHD* tS_pMdToHd)
{
	...
	{
		ADC_CHN0 = ADC0_CURRETN_B_CHANNEL | (ADC0_CURRETN_B_CHANNEL << 8);
		ADC_CHN1 = ADC0_CURRETN_B_CHANNEL | (ADC0_CURRETN_B_CHANNEL << 8);
	}
...
{
    tS_pHd2Md->bBreakInStatus = (MCPWM_EIF & BIT4);
}
...
void EPWM0_Charge(u8 t_bPhaseABC)
{
	...
	{
		MCPWM_IO01		= 0x0C0C | g_hDriverPolarity[0];
...
/*******************************************************************************
 函数名称：    void getEpwm0DefaultSampoint0(void)
...
/*******************************************************************************
 函数名称：    void getEpwm0DefaultSampoint1(void)
