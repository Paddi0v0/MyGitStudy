...
void ADC_IRQHandler(void)
{
	...
	if(microCheck.Insert_err_flg & INTERRUPT_ERROR)
	{//中断自检
		microCheck.Interrupt_ADC_cnt = 0;
	}
	else if(microCheck.Interrupt_TIMER_cnt < 6)
	{
		microCheck.Interrupt_ADC_cnt++;
	}

	if(microCheck.Insert_err_flg & ADC_ERROR)
	{
		microCheck.REF_ADC = 1450;
		microCheck.VSS_ADC = 30;
	}
	else
	{
		microCheck.REF_ADC = GET_MCU_VREF >> 4;
		microCheck.VSS_ADC = GET_MCU_VSS >> 4;
	}
	...
	#if(RTT_FUNCTION == FUNCTION_ON)
	Rttstru.data0 = getElectAngleEst(0);
	Rttstru.data1 = getPhaseCur_U(0);
...
{
	//PWM0_时基0中断标志
	MCPWM_IF0 = BIT1 | BIT0;  //T1事件|T0事件   注意，触发了ADC0 通道采样
...
void TIMER0_IRQHandler(void)
{
	...
	/***********ClassB Code******************/
	#if (CLASSB_ENABLE == FUNCTION_ON)
    if(microCheck.Interrupt_TIMER_cnt < 200)//100ms
    {
        microCheck.Interrupt_TIMER_cnt++;
    }
    else
    {
        Interrupt_Check();
