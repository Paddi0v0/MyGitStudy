...
#define   SAFTEY_MEMORY_ADDR       0x7A00             //MarchC+检测结果和flash校验golden结果存储地址，此地址一定要大于FLASH_SIZE
#define   ROM_INSERT_ERR_ADDR      0x7000             //ROM自检插入故障时的flash地址，此地址一定要小于FLASH_SIZE且是空闲没有被代码占用的

#define   CRC_RESULT_BUFFER        0x20000900         //flash crc自检中间结果存储RAM地址

#undef    RAM_SIZE
#undef    FLASH_SIZE
#define   RAM_SIZE                 0x400              //要检测的RAM空间，LKS08x 最大设置0x1FFE,LKS05x 最大设置0x9FE,LKS03x 最大设置0xFFE,
#define   RAM_ITER                 (RAM_SIZE >> 1)    //(RAM_SIZE>>1)

#define   FLASH_SIZE               0x7400             //flash CRC校验的空间范围，CRC每次检测32位数据
#define   FLASH_ITER               (FLASH_SIZE >> 2)  //FLASH_SIZE/4
...
#define   WDT_WAIT_CNT             4096               //设置看门狗递减次数，固定4096
...
extern const Software_SafteyStruct struSaftey;
