...
;PS：此处08x、05x、03x略有不同,需要设置不同的结束地址	
	;令R3为RAM结束地址,
    MOVS    R3, #1          ;R3赋值为1                   
    LSLS    R3, R3, #12     ;R3 = 0x1000       R3向左移11位
	                     		
    SUBS    R3, #2          ;R3 = 0xFFE
	
    ADD     R3,R0,R3	    ;R3 = 0x20000FFE，令R3等于RAM结束地址	
...
RAM_Clear0Rise
	...
	CMP     R0, R3          ;先地址自加再执行循环，故执行到R0 = 0x20000FFE循环结束
    BNE     RAM_Clear0Rise  ;R1 - R3 ≠ 0   执行 RAM_Clear0Rise  

;2、升序检测RAM是否为0
	MOV     R0,R4           ;R0 = 0x20000000
	ADDS    R3,R3,#2        ;R3 = R3+ 2  R3 = 0x20001000
RAM_Test0Rise	
    MOV     R1,R0           ;R1 = R0
...
;    MOVS    R1,#5          ;	                                                              2、加入错误	
	...
	ADDS    R0,R0,#2        ;R0 = R0+ 2
	CMP     R0,R3           ;先执行循环，再地址自加进行判断，故执行到R0 = 0x20001000时循环结束
...
;6、降序检测RAM是否为0
    SUBS    R3, R3, #2      ;R3 = R3-2 = 0x20000FFE，降序时最先执行的地址是0x20000FFE
	SUBS    R4, R4, #2      ;降序检测时检测到0x1FFFFFFE循环结束
	MOV     R0, R3          ;R0 = 0x20000FFE,从0x20000FFE开始降序往下读值检测，赋值检测
RAM_Test0Fall	
    MOV     R1,R0           ;R1 = R0
    LDRH    R1,[R1]	        ;将R1地址的数据赋给R1,并将R1的高16位清零
;    MOVS    R1,#5          ;	                                                                5、加入错误	
...
;8、降序检测RAM是否为1
	MOV     R0,R3           ;R0 = 0x20000FFE，降序时最先执行的地址是0x20000FFE	
RAM_Test1Fall	
    MOV     R1,R0           ;R1 = R0
    LDRH    R1,[R1]	        ;将R1地址的数据赋给R1,并将R1的高16位清零
;    MOVS    R1,#5          ;	                                                                7、加入错误
