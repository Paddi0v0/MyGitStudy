...
static void UAStopCmdHandle(pSTR_UACtrProc pObj)
{
	...
	updateUASetpoint(pObj);										//	UA目标指令处理函数
	pObj->mUaToMa.eBrakeMode	= getBrakeMode4UA(pObj);		//	UA刹车指令处理函数
...
/*******************************************************************************
 函数名称：    UAClrFaultCmdHandle(pSTR_UACtrProc pObj) 
 功能描述：    故障清除函数
 输入参数：    pSTR_UACtrProc 
 输出参数：    无
 返 回 值：    无
...
/*******************************************************************************
 函数名称：    UAStoppingCmdHandle(pSTR_UACtrProc pObj) 
 功能描述：    停止处理函数
 输入参数：    pSTR_UACtrProc 
 输出参数：    无
 返 回 值：    无
...
static void UAStoppingCmdHandle(pSTR_UACtrProc pObj)
{
	pObj->mUaToMa.eBrakeMode	= getFaultBrakeMode4UA(pObj);	//	UA故障指令处理函数
	if(pObj->mUaToMa.eBrakeMode == E_STOP_MOE_OFF)
	{
		pObj->mUaToMa.eAppCmd	= E_CMD_MOTOR_STOP_MOE_OFF;		//	关闭MOE
	}
	else
...
static void UAFaultStopHandle(pSTR_UACtrProc pObj)
{
	...
	if(pObj->mUaToMa.eBrakeMode == E_STOP_MOE_OFF)
	{
		pObj->mUaToMa.eAppCmd	= E_CMD_MOTOR_FAULT_DIMOE;		//	FAULT停止电机,MOE关闭
	}
	else
	{
		pObj->mUaToMa.eAppCmd	= E_CMD_MOTOR_FAULT_ENMOE;		//	FAULT停止电机,MOE使能
	}
