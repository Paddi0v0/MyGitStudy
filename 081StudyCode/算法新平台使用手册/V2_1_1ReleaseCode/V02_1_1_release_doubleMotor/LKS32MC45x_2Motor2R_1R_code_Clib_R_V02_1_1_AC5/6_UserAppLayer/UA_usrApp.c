...
{	
	...
	if (pObj->m_bLoopMode == SPEED_LOOP)         //速度环
	{
		if(pDrvCfgPara->mS_GlobalCfg.m_buPowerLmtInSpeedmode)    //速度环限功率
		{
			pObj->m_wSetpoint		= pDrvCfgPara->mS_FBSpdLoop.m_wPowerLmtSpdValue;	//速度设定
			pObj->m_wPowerSetpoint	= pDrvCfgPara->mS_FBSpdLoop.m_wPowerLmtValue;		//功率设定
		}		
		pObj->m_wTqSetpoint = pDrvCfgPara->mS_FBCurLoop.m_wIqSet;						//电流设定
	}

	else if (pObj->m_bLoopMode == CURRENT_LOOP)  //电流环
	{
		pObj->m_wTqSetpoint = pDrvCfgPara->mS_FBCurLoop.m_wIqSet;
	}
	
	else if (pObj->m_bLoopMode == POWER_LOOP)   //功率环
	{	
		if(pDrvCfgPara->mS_GlobalCfg.m_buSpdLmtInPowermode)      //功率环限速度
		{
			pObj->m_wPowerSetpoint	= pDrvCfgPara->mS_FBPwrLoop.m_wLmdSpdPowerSetting;	//功率设定
			pObj->m_wSetpoint		= pDrvCfgPara->mS_FBPwrLoop.m_wSpdLmtValue;			//速度设定
		}
		else
		{
		pObj->m_wPowerSetpoint	= pDrvCfgPara->mS_FBPwrLoop.m_wPowerSetting;           //功率设定
		}		
		pObj->m_wTqSetpoint = pDrvCfgPara->mS_FBCurLoop.m_wIqSet;                      //电流设定
	}
...
ENUM_MotorBrakeMode getBrakeMode4UA(pSTR_UACtrProc pObj)
{
	if(pObj->m_bStopMode == 1)
	{
		return E_STOP_ZERO_CUR;       //零电流关断
	}
	else if(pObj->m_bStopMode == 2)
	{
		return E_STOP_SHORT;         //下桥臂短接
	}
	else
