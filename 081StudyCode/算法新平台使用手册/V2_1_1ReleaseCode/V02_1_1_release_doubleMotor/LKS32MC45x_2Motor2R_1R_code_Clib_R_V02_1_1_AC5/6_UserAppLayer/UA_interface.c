...
pSTR_UACtrProc	get_UA_CtrPtr(u8 ObjID)
{
	if(ObjID < UA_OBJ_MAX_DEFINE)
	{
		return &sS_UACtrlProc[ObjID];
	}
	else
	{
		return NULL;
...
static void handleUACtrlObj(u8 nuObjID)
{
	...
	pObj	= get_UA_CtrPtr(nuObjID);
		
	#ifdef UA_DEBUG			//	可根据需要修改命令传递方式等函数的实现
	if(nuObjID == 0)
	{
		updateM0DebugUA(pObj);	/* UA层M0调试指令传递 */	
	}
	else if(nuObjID == 1)
	{
		updateM1DebugUA(pObj);	/* UA层M1调试指令传递 */	
	}
...
static bool initUACtrlObj(void)
{
	u8 t_nuObjNbr;
	pSTR_UACtrProc pObj;
	u8 t_nuMaxObjNbr = get_Max_UAObj_Nbr();
	bool result = FALSE;	

	for(t_nuObjNbr = 0; t_nuObjNbr < t_nuMaxObjNbr; t_nuObjNbr++)
	{
		pObj	= get_UA_CtrPtr(t_nuObjNbr);             //按控制对象ID进行查询控制对象指针
		if(pObj != NULL)
		{	
			memset(pObj, 0, sizeof(pSTR_UACtrProc));     //全局配置初始化
			pObj->mg_nUACtrlProcID	= t_nuObjNbr;        //当前UA控制对象ID传递
			init_UA_Crtl_Proc(pObj);                     //按控制对象进行UA初始化处理
		}
	}
...
{
	u8 t_nuObjNbr;
	u8 t_nuMaxObjNbr = get_Max_UAObj_Nbr();     //查询控制对象个数

	pSTR_UACtrProc pObj;
	bool b_MotorRunning = FALSE;
	...
	for(t_nuObjNbr = 0; t_nuObjNbr < t_nuMaxObjNbr; t_nuObjNbr++)
	{
		...
		if(pObj != NULL)
		{
			if(!b_MotorRunning)
			{
				b_MotorRunning = (pObj->m_eUASysState == E_UA_RUN);  // UA状态机运行状态 
			}
		}
		else
		{
...
void UACtrlObjTask(void)
{
	...
	#ifdef _POLLING_MODE
	t_nuObjID++;
	if(t_nuObjID >= t_nuMaxObjNbr)
	{
		t_nuObjID	= 0;
	}
	handleUACtrlObj(t_nuObjID);					//按控制对象ID进行UA任务处理
	
	#else

	for(t_nuObjNbr = 0; t_nuObjNbr < t_nuMaxObjNbr; t_nuObjNbr++)
	{
		handleUACtrlObj(t_nuObjNbr);			//按控制对象ID进行UA任务处理
	}
	#endif
