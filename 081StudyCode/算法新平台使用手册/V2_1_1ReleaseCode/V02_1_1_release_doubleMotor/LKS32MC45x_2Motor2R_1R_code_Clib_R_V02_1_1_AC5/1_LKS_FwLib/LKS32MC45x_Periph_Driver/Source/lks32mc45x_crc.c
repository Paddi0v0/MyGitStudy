...
void CRC_Init(CRC_TypeDef *CRCx, CRC_InitTypeDef *this)
{
    ...
    CRC_Enable(CRCx);
    CRC_CR = (u32)(this->REFOUT << 12) | (u32)(this->REFIN << 8) | (u32)(this->WIDTH << 4);
    CRC_CR |= BIT0; // 复位
    CRC_POL = this->POLY;
    CRC_INIT = this->INIT;
...
u32 CRC_Calc(CRC_TypeDef *CRCx, u8 *data, u32 len)
{
    ...
    return (REG32(&CRCx->DR)) ^ CRC_XOROUT;
}
/**
 * @brief CRC计算接口
 */
...
void CRC_Enable(CRC_TypeDef *CRCx)
{
    if (CRCx == CRC0)
    {
        SYS_ModuleClockCmd(SYS_MODULE_CRC, ENABLE);
    }
}
/**
 * @brief CRC模块时钟关闭
...
void CRC_Disable(CRC_TypeDef *CRCx)
{
    if (CRCx == CRC0)
    {
        SYS_ModuleClockCmd(SYS_MODULE_CRC, DISABLE);
    }
}
