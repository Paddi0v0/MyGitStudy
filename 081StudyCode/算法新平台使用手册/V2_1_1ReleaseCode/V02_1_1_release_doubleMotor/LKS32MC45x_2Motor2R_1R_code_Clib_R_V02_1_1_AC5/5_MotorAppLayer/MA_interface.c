...
{
	u8 t_nuObjNbr;
	u8 t_nuMaxObjNbr = getMaxMAObjNbr();
...
bool initMACtrlObj(void)
{
	...
	result = registerSubBlock2MACrtlProc();			//	多电机 注册内部变量及指针、初始化内部变量
	if(result)
	{
		for(t_nuObjNbr = 0; t_nuObjNbr < t_nuMaxObjNbr; t_nuObjNbr++)
		{
			initMACtrProc(t_nuObjNbr);				//	配置初始化
		}
	}
	return result;
}
