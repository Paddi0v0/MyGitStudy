...
extern STR_MotorFocElement		gS_FocElementObjM[MAX_DRV_MODULE_USED];
extern u8			g_buMdPWMONDelay[MAX_DRV_MODULE_USED];
/*******************************************************************************/
//外部函数引用区

extern	void	InitFocVar(u8 t_bObjId);

static inline void	AdcCurSampleCalc(u8 t_bObjId);
...
extern	s16		getBemfTrackMinRecovVolt(u8 t_objId);
...
extern	s16		getid(u8 t_bObjId);
extern	s16		getiq(u8 t_bObjId);
extern	s16		getEstSpeed(u8 t_bObjId);

/*******************************************************************************
 函数名称：    void AdcSampleCal(u8 t_bObjId)
 功能描述：    电流采样
 输入参数：    u8 t_bObjId
 输出参数：    无
 返 回 值：    无
...
static inline void AdcCurSampleCalc(u8 t_bObjId)
{
    ...
    this = &gS_FocObjM[t_bObjId].mCurSamplingRes;  //指针
    STR_VectorUVW*	t_pStatCurrUVW	= &gS_FocElementObjM[t_bObjId].mStatCurrUVW;
	STR_VectorAB *	t_pStatCurrAB	= &gS_FocElementObjM[t_bObjId].mStatCurrAB;
    ...
    /* clark变换 */
	t_pStatCurrAB->nBeta = _IQ15mpy(18919, (t_pStatCurrUVW->nPhaseV - t_pStatCurrUVW->nPhaseW));
	t_pStatCurrAB->nAlph = t_pStatCurrUVW->nPhaseU;
}
...
static inline void updateMd2MaVar(PSTR_MotorFoc tS_pMotorFoc)
{
	...
	tS_pMotorFoc->pMdToMa->wFreq	= getEstSpeed(t_bObjId);							//	估算转速(频率)
	tS_pMotorFoc->pMdToMa->nVdc		= tS_pMotorFoc->pHdToMd->nDcVolAdcDat;				//	直流电压采样值

	tS_pMotorFoc->pMdToMa->mStatCurrUVW.nPhaseU	= gS_FocElementObjM[t_bObjId].mStatCurrUVW.nPhaseU;
	tS_pMotorFoc->pMdToMa->mStatCurrUVW.nPhaseV = gS_FocElementObjM[t_bObjId].mStatCurrUVW.nPhaseV;
	tS_pMotorFoc->pMdToMa->mStatCurrUVW.nPhaseW = gS_FocElementObjM[t_bObjId].mStatCurrUVW.nPhaseW;
	...
	#if ( (MAX_DRV_MODULE_USED>1) || ((MAX_DRV_MODULE_USED==1) &&(DRV0_BEMF_CHECK_EABLE==FUNCTION_ON)) )
	...
	tS_pMotorFoc->pMdToMa->mStatCurDQ.nAxisD	= getid(t_bObjId);
	tS_pMotorFoc->pMdToMa->mStatCurDQ.nAxisQ	= getiq(t_bObjId);
}

/*******************************************************************************
 函数名称：    static inline void updateMa2MdVar(u8 t_ObjId)
 ...
 2023/9/05      V1.1            Tonghua.Li          			  修改注释
 *******************************************************************************/
static inline void updateMa2MdVar(u8 t_ObjId)
{
	gS_FocElementObjM[t_ObjId].mStatCurDQCmd.nAxisD		= gS_FocObjM[t_ObjId].pMaToMd->mStatCurDQCmd.nAxisD;
	gS_FocElementObjM[t_ObjId].mStatCurDQCmd.nAxisQ 	= gS_FocObjM[t_ObjId].pMaToMd->mStatCurDQCmd.nAxisQ;
	gS_FocElementObjM[t_ObjId].mMotorSpeed.wSpeedRef	= gS_FocObjM[t_ObjId].pMaToMd->wMotorSpeedRef;	/* 转速指令 */
}
...
/*******************************************************************************
 ...
 2023/9/05      V1.1            Tonghua.Li          			  修改注释
 *******************************************************************************/
static inline void RegMotorCurrentDQInline(STR_MotorFocElement *tS_pFocElememt)
{
	if(!gS_FocObjM[tS_pFocElememt->m_bObj_ID].bObserWork)
	{
		#if (VF_START_FUNCTION_ENABLE == FUNCTION_ON)		
		if(!getCurLoopCloseStatusFromOB(tS_pFocElememt->m_bObj_ID))	//电流环开环
		{
			tS_pFocElememt->mAcrD.nError = 0;
			tS_pFocElememt->mAcrQ.nError = 0;
		}
		else														//电流环闭环
		#endif
		{		
			tS_pFocElememt->mAcrQ.nError = (s32)tS_pFocElememt->mStatCurDQCmd.nAxisQ - tS_pFocElememt->mStatCurDQ.nAxisQ; /* Q轴输入误差 */
			tS_pFocElememt->mAcrD.nError = (s32)tS_pFocElememt->mStatCurDQCmd.nAxisD - tS_pFocElememt->mStatCurDQ.nAxisD;  /* D轴输入误差 */
		}
		tS_pFocElememt->mAcrOutDQ.nAxisQ = CurrentPIRegulator(&tS_pFocElememt->mAcrQ);		/* Q轴电流PI计算，输出Vq */
		tS_pFocElememt->mAcrOutDQ.nAxisD = CurrentPIRegulator(&tS_pFocElememt->mAcrD);		/* D轴电流PI计算，输出Vd */
	}

	//	电压极限圆控制，内部包含自动过调制功能
	CalcVolCircle(tS_pFocElememt->m_bObj_ID,&tS_pFocElememt->mAcrOutDQ,&tS_pFocElememt->mStatVolAB);
}
...
{	
	...
	{
		InitFocVar(t_bObjId);							//电流环变量初始化
		InitPmFluxOB(t_bObjId);							//观测器变量初始化
		#if ( (PLANTFORM_DRV_MODULE_NUM>1) || ((PLANTFORM_DRV_MODULE_NUM==1) &&(DRV0_DIR_CHECK_ENABLE==FUNCTION_ON)) )	
		...
		Bemf_CheckInit(	t_bObjId,
						getBemfAngComp(t_bObjId),
						getBemfTrackMinVolt(t_bObjId),
						getBemfTrackMinRecovVolt(t_bObjId)
					  );   /*反电势检测变量初始化*/	
		#endif
...
/*******************************************************************************
 函数名称：    static void FOC_Model(u8 t_bObjId) 
 功能描述：    PMSM电机Q轴转矩控制，D轴电流控制处理，实现FOC控制的核心电流环
 输入参数：    u8 t_bObjId    FOC对象结构体 ID
 输出参数：    无
 返 回 值：    无
 其它说明：
 修改日期      版本号          修改人            修改内容
 ...
 *******************************************************************************/
static inline void FOC_Model(u8 t_bObjId) 
{	
	...
	PSTR_VectorAB			t_pStatCurrAB	= &gS_FocElementObjM[t_bObjId].mStatCurrAB;	//传递电流AB指针
	STR_TrigComponents		*t_pSinCos		= &gS_FocElementObjM[t_bObjId].mSinCosFocAngle;	//sin cos查表计算
	STR_PubDatMAMD*		t_pMaToMd		 =  tS_pMotorFoc->pMaToMd;			//传递FOC变量指针

	AdcCurSampleCalc(t_bObjId);            /*电流采样函数计算*/
	...
	switch(t_eMotorStatus)
	{
		...
		{			
			...
			tS_pMotorFoc->m_pHALHandle->MCPWMx_RegUpdate(tS_pMotorFoc->pMdToHd);  /* 加载PWM模块占空比值 */
				...
				#endif
			#else
				if( (tS_pMotorFoc->m_pSvpwmPar->bSampType == CURRENT_SAMPLE_3SHUNT)
					||(tS_pMotorFoc->m_pSvpwmPar->bSampType == CURRENT_SAMPLE_MOSFET)
					)													/* 三电阻采样或MOS内阻采样 */
				{
				tS_pMotorFoc->pMdToHd->mMcTmr.eChannel = E_PHASE_A_B;	/* 设置采样通道为AB */
				tS_pMotorFoc->m_pHALHandle->AD_ChannelUpdate(tS_pMotorFoc->pMdToHd);  /* 配置AD通道 */
				tS_pMotorFoc->mCurSamplingRes.mCurSamplingEvent.pAdcSamp	 = (ADCSampFunction)&SVPWM_3ShuntGetPhaseIaIb;
				}
			#endif
		...
		case E_MOTOR_BEMF_TRACKING:     /* 反电势顺逆风检测 */
		{
			...
			tS_pMotorFoc->m_pHALHandle->AcquireBemf(tS_pMotorFoc->pHdToMd);		
		...
		{			
			...
			{
				tS_pFocElememt->mMotorAngle.wElectAngle	 = tS_pFocElememt->mMotorAngle.wElectAngleEst;

				#if (MCU_DSP_SINCOS == FUNCTION_OFF)
				Trig_Functions((tS_pFocElememt->mMotorAngle.wElectAngle >> 16),&tS_pFocElememt->mSinCosFocAngle); /* sincos查表 */
				#else
				DSP_SC |= BIT2;
    			DSP_CORDIC_THETA = (tS_pFocElememt->mMotorAngle.wElectAngle >> 16);                         /* sincos通过DSP计算 */
				t_pSinCos->hCos						= DSP_CORDIC_COS;
				t_pSinCos->hSin						= DSP_CORDIC_SIN;
				#endif

				// alph/beta to dq 坐标变换		
				tS_pFocElememt->mStatCurDQ.nAxisQ	= (((s32)t_pStatCurrAB->nBeta * t_pSinCos->hCos) -
				                        ((s32)t_pStatCurrAB->nAlph * t_pSinCos->hSin)) >> 15;
				tS_pFocElememt->mStatCurDQ.nAxisD	= (((s32)t_pStatCurrAB->nAlph* t_pSinCos->hCos) +
				...
				// 观测器计算函数
				PmsmFluxObserve_Full(t_bObjId,&tS_pFocElememt->mStatCurDQ);
			}			
		break;
		}
		...
		{   /* PWM brake */
			...
			// 根据采样方式设置默认采样状态
			#if (MAX_DRV_MODULE_USED == 1)                                          /* 单电机采样处理 */
				#if ((DRV0_CUR_SAMP_TYPE == CURRENT_SAMPLE_3SHUNT) || (DRV0_CUR_SAMP_TYPE == CURRENT_SAMPLE_MOSFET))
				...
				#endif
			#else
				if( (tS_pMotorFoc->m_pSvpwmPar->bSampType == CURRENT_SAMPLE_3SHUNT)   /* 双电机采样处理 */
					||(tS_pMotorFoc->m_pSvpwmPar->bSampType == CURRENT_SAMPLE_MOSFET)
					)
				{
				tS_pMotorFoc->pMdToHd->mMcTmr.eChannel = E_PHASE_A_B;                     /* 采样模式选择 */
			...
			InitFocVar(t_bObjId);				/* FOC参数初始化 */
			InitPmFluxOB(t_bObjId);				/* 观测器参数初始化 */
			break;
		}
		
		#if (((PLANTFORM_DRV_MODULE_NUM==1) &&(DRV0_POS_SEEK_ENABLE==FUNCTION_ON)) )    /* 单电机IPD处理 */
		case E_MOTOR_IPD:         /*IPD初始位置检测*/
		{
			  ...
			  )
			{
				if(tS_pMotorFoc->m_bIPDFinsh == 0)
				{
					...
					tS_pMotorFoc->nuIPDAng		= getIPDAngleLib(t_bObjId);
					tS_pMotorFoc->m_bIPDFinsh 	= getIPDFinishFlag(t_bObjId);
					__enable_irq();                   /* 开启总中断 */
		...
		#elif (PLANTFORM_DRV_MODULE_NUM > 1)	/* 双电机IPD处理 */
		case E_MOTOR_IPD:          				/*IPD初始位置检测*/
		{
...
#if (MAX_DRV_MODULE_USED == 1)	/* 单电机1ms任务处理函数 */ 
void MDtaskHandleIn1msLoop(void)
{
	...
	/* 数据指针传递 */
	updateMa2MdVar(0);								//更新MA to MD 状态及电压速度等信息
	...
	if(tS_pMotorFoc->pMaToMd->eMotorStatus != E_MOTOR_TRACKING)
	{
		vectorVFcontrol(0,&gS_FocElementObjM[0].mAcrD.nComp,&gS_FocElementObjM[0].mAcrQ.nComp);
	}
	#endif
}
...
void MDtaskHandleIn1msLoop(void)
{
	...
	for(t_bObjId = 0; t_bObjId < MAX_DRV_MODULE_USED; t_bObjId++)
	{
		...
		updateMa2MdVar(t_bObjId);			//更新MA to MD 状态及电压速度等信息
		
		RespondPWMStatus(tS_pMotorFoc);			// 反馈MA层MOE状态
		RespondMotorStatus(tS_pMotorFoc);		// 反馈MA层电机状态
		updateMd2MaVar(tS_pMotorFoc);			// 更新MD到MA层电机状态及信息

		PmsmFluxObserve1ms(t_bObjId);			// 观测器慢速1ms处理函数
		...
		if(tS_pMotorFoc->pMaToMd->eMotorStatus != E_MOTOR_TRACKING)
		{
			vectorVFcontrol(t_bObjId,&gS_FocElementObjM[t_bObjId].mAcrD.nComp,&gS_FocElementObjM[t_bObjId].mAcrQ.nComp);
		}
		#endif
	}
}
...
/*******************************************************************************
 函数名称：    void AdcEocIsrDRV1(void) 
 功能描述：    PMSM PWM1 FOC控制
 输入参数：    无
 输出参数：    无
 返 回 值：    无
 其它说明：
 ...
 2020/12/15      V1.1       Tonghua Li				完善HAL层驱动接口
 *******************************************************************************/
void AdcEocIsrDRV1(void)
{
	PSTR_MotorFoc tS_pMotorFoc;
	STR_PubDatHDMD* tS_pCurAdcRes;
	tS_pMotorFoc = &gS_FocObjM[1];
	tS_pCurAdcRes = tS_pMotorFoc->pHdToMd;
	...
	tS_pMotorFoc->m_pHALHandle->AcquireVdc(tS_pCurAdcRes);
	#else   //内联函数读取AD数值，速度快
	AcquireCurSamplingResultsDRV1(tS_pCurAdcRes);
	AcquireVdcSamplingResultsDRV1(tS_pCurAdcRes);	
	#endif
	...
	if(tS_pMotorFoc->pMaToMd->eMotorStatus == E_MOTOR_SYN)
	{
		FaultCurrentCheck(tS_pMotorFoc->m_pStatCurrUVW, tS_pMotorFoc->m_bPWM_ID);
	}
	#endif
}
