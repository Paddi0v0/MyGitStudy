...
extern void CalcVolCircleComp(u8 t_bObjId,PSTR_VectorDQ tS_pVolDq,PSTR_VectorAB tS_pVolAB); //添加死区补偿
...
extern void SVPWM_1Shunt_5_7Seg(u8 t_bObjId,PSTR_VectorAB tS_pVolAB);	//单电阻5/7自动切换

extern void SVPWM_1Shunt_En(u8 t_bObjId,PSTR_VectorAB tS_pVolAB);		//单电阻增强

extern void SVPWM_1Shunt_Mode0(u8 t_bObjId,PSTR_VectorAB tS_pVolAB);	//单电阻 模式0
extern void SVPWM_1Shunt_Mode1(u8 t_bObjId,PSTR_VectorAB tS_pVolAB);	//单电阻 模式1
...
// 单电机应用 IPD检测函数
// 关中断操作，soft_Trg_ADC_DRV0需要配置为对应的ADC函数
extern void IPDEst_SMotor(u8 t_bObjId);				//	单电机IPD函数
extern void IPDEst_SMotorPWM1(u8 t_bObjId);			//	单电机IPD函数,采用PWM1以及HALL1
...
extern s16	getMotorActVoltFromObjId1ms(u8 t_bObjId);			// 当前电机电压
extern s16	getPhaseCur_U(u8 t_bObjId);							// 获取U相电流值
extern s16	getPhaseCur_V(u8 t_bObjId);							// 获取V相电流值
extern s16	getPhaseCur_W(u8 t_bObjId);							// 获取W相电流值
extern s16	getiq_ref(u8 t_bObjId);								// 获取iqRef电流值
extern s16	getid_ref(u8 t_bObjId);								// 获取idRef电流值
extern s16	getid(u8 t_bObjId);									// 获取id
extern s16	getiq(u8 t_bObjId);									// 获取iq
extern s16	getUd(u8 t_bObjId);									// 获取电流环PI输出Ud
extern s16	getUq(u8 t_bObjId);									// 获取电流环PI输出Uq

extern s16	getVdc(u8 t_bObjId);   								// 获取Vdc(usr数据格式)
...
//  nBemfTrackVoltRecover	----- 检测恢复电压，0.01V
extern void Bemf_CheckInit(u8 objId , u16 nCompAngle, u16 nBemfTrackVolt, u16 nBemfTrackVoltRecover);

//反电势检测
extern void Bemf_CheckProcess(u8 objId, s16 phaseC, s16 phaseB, s16 phaseA);
extern void Bemf_CheckProcessEn(u8 objId, s16 phaseC, s16 phaseB, s16 phaseA);
//  phaseC   ------  AD原始值

//反电势切换到正常运行, isWeakenUsed  TRUE ---- 弱磁观测器
extern void BemfSwitch(u8 objId, bool isWeakenUsed);

//反电势切换到正常运行(抵消VF影响)，isWeakenUsed  TRUE ---- 弱磁观测器
extern void BemfSwitchWithVF(u8 objId, bool isWeakenUsed);
...
// 设置设置外部速度到库模块 core数据格式
extern	void setEstSpdFromExternalModule(u8 t_bObjId, s32 estSpd);
// 设置闭环状态到库模块
extern	void setVCState2RunStatus(u8 t_bObjId,bool iscloseLoop);

// 慢速电压计算函数，可放置低优先级函数中
extern	void CalcDcVoltPerUnitSlowTask(u8 t_bObjId,s16 t_nUdcAdcRes);

extern	bool	getIPDFinishFlag(u8 t_bObjId);					// IPD结束标志
extern	u16		getIPDAngleLib(u8 t_bObjId);

extern	s16		getMotorVoltFromObjId(u8 t_bObjId);				// SQRT(Ud^2+Uq^2)


extern	s16		getMotorDaxisVoltFormObjId(u8 t_bObjId);		// d axis实际电压
extern	s16		getMotorQaxisVoltFormObjId(u8 t_bObjId);		// q axis实际电压
extern	s16 	getElectBmefD(u8 t_bObjId);						// d axis反电势
extern	s16 	getElectBmefQ(u8 t_bObjId);						// q axis反电势

extern	s16		getIdEstFromObjId(u8 t_bObjId);					// d axis估算电流
extern	s16		getIqEstFromObjId(u8 t_bObjId);					// q axis估算电流

extern	s32		getOnFlyBemfFreqResultFromObjId(u8 t_bObjId);	// 反电势顺逆风估算频率
extern	s16		getElectSpdEst(u8 t_bObjId);					// 观测器估算频率(速度)

extern	u16		getElectAngle(u8 t_bObjId);						// 当前电角度
extern	u16		getElectAngleEst(u8 t_bObjId);					// 估算电角度
