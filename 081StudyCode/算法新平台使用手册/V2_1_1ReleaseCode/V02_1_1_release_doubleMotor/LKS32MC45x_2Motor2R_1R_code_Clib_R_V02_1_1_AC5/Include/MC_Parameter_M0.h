...
/* -----------------------------Hardware Parameter---------------------------- */
#define ADC_SUPPLY_VOLTAGE_M0             (3.3)              /* 单位: V  ADC基准电压，3.6或者2.4,大部分应用选择3.6 */
//双电阻采样范围:3.3/(6.667*0.05)=10A
//电阻2K欧 80/(10+1*2)=6.667   当前2K欧姆

//单电阻采样范围:3.3/(6.667*0.05)=10A
//电阻1K欧 80/(10+1)=6.667   当前2K欧姆
#define AMPLIFICATION_GAIN_M0			(6.667)             /*运放放大倍数*/  
...
/* -------------------------------- 顺逆风检测参数------------------------------ */
#define SPEED_TRACK_DELAYTIME_M0			(100)			/* 单位: ms 顺逆风检测时间  */
...
#define BEMF_TRACK_RECOV_VOLT_M0			(300)			/* 单位: 0.01V 反电势检测恢复电压，根据实际负载修改。高于该电压时进行顺逆风检测 */



/*------------------------------------预定位参数---------------------------------*/
#define ALIGN_ANGLE_M0						(0.0)			/* 单位:度 预定位角度 */
#define U_START_CUR_SET_F_M0				(0.5)			/* 单位: A 第一段定位电流 */
#define U_START_CUR_SET_S_M0				(1.0)			/* 单位: A 第二段定位电流 */
...
#define ALIGN_CURRENT_ACC_M0				(30)			/* 单位: A/s  定位电流加速调整值  初始位置检测使能后给到最大值，不能超过30，否则数据会溢出。 */
#define ALIGN_CURRENT_DEC_M0				(30)			/* 单位: A/s  定位电流减速调整值  初始位置检测使能后给到最大值，不能超过30，否则数据会溢出。 */
...
#define FREQ_ACC_M0							(10.0)			/* 单位：Hz/s 开环拖动频率加速调整值 */
#define FREQ_DEC_M0							(10.0)			/* 单位：Hz/s 开环拖动频率减速调整值 */
...
#define CURRENTLOOP_CLOSE_FREQ_M0			(0.0)			/* 单位：Hz 电流环闭环频率 */
...
#define ID_START_M0							(0.0)			/* 单位:A, D轴电流设定值 */
...
#define P_ASR_KP_M0							(2000)			/* 速度环Kp */
#define P_ASR_KI_M0							(1000)			/* 速度环Ki */

#define IQMAX_M0							(2.0)			/* 单位:A, 速度环输出最大值 */
#define IQMIN_M0							(-2.0)			/* 单位:A, 速度环输出最小值 */

#define SPEED_RUN_ACC_M0					(150.00)			/* 单位 Hz/s 速度加速调整值 */
#define SPEED_RUN_DEC_M0					(150.00)			/* 单位 Hz/s 速度减速调整值 */
...
#define U_OV_VOLT_PROTECT_ENABLE_M0 		(0)					/* 过压报警保护使能 */
#define U_UND_VOLT_PROTECT_ENABLE_M0 		(0)					/* 低压报警保护使能 */
