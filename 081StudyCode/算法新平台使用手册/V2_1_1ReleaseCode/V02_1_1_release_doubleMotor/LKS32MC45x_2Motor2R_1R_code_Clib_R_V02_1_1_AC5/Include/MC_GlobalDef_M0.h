...
#define OPA_SELECT_M0						(2)						/*	运算放大倍数 选择 */
...
#define VF_DEADZONE_COMP_FACTOR_M0			(950)		// Q15格式: 0~32767 	0----- 根据死区时间自动计算
...
/* ----------------------VF 启动是否是能---------------- */
#define VF_START_STATUS_M0					(FALSE)					/* VF 启动状态 TRUE 为使能, FALSE 为不使能, 当前未用，预留 */

/* ----------------------AC检测及保护是否是能---------------- */
#define AC_SENOR_PROTECT_STATUS_M0			(FALSE)					/* AC检测及保护状态 TRUE 为使能, FALSE 为不使能,当前未用，预留  */
...
/*----------------------------------速度环参数-------------------------------*/
#define POWER_LIMIT_STATUS_M0             (TRUE)			/* 限功率状态，TRUE = 使能，FALSE = 不使能 */
