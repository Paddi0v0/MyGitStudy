...
#define OPA_SELECT_M1						(4)						/*	运算放大倍数 选择 */
...
#define DEADTIME_M1                          (u16)(((unsigned long long)PWM_MCLK_M1 * (unsigned long long)DEADTIME_NS_M1)/1000uL)

#define ADC_COV_TIME_M1						(200)   /* 预留的ADC转换时间，单位：500--200ns*/
#define SAMP_STABLE_TIME_SHUNT_M1			(1500)  /* 1000--1800单电阻采样，信号稳定时间设置| 单位 nS */
...
#define SEEK_POSITION_STATUS_M1				(FALSE)					/* 初始位置检测状态 TRUE 为使能, FALSE 为不使能 */

/* ----------------------顺逆风检测是否是能---------------- */
#define DIR_CHECK_STATUS_M1					(FALSE)					/* 顺逆风检测状态 TRUE 为使能, FALSE 为不使能 */

/* ----------------------allign 对相是否是能---------------- */
#define ALLIGN_STATUS_M1					(FALSE)					/* ALLIGN对相状态 TRUE 为使能, FALSE 为不使能 */
...
/*------------------------------------功率环参数-------------------------------*/
#define SPPED_LIMIT_STATUS_M1            (TRUE)			/*  限转速状态，TRUE = 使能，FALSE = 不使能 */

/*------------------------------------弱磁控制-------------------------------*/
#define WEAKEN_FLUX_STATUS_M1             (TRUE)			/*  弱磁控制状态，TRUE = 使能，FALSE = 不使能 */
