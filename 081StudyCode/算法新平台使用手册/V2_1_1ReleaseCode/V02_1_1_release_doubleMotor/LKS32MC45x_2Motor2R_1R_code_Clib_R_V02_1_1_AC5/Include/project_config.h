...
#define PLANTFORM_MAX_PWM_MODULE_NUM	(2)			// 设定实际的PWM组数
...
*        EPWM1对应的AD支持 单电阻
**********************************************************************************/
#define EPWM0_CURRENT_SAMPLE_TYPE		(CURRENT_SAMPLE_2SHUNT)
#define EPWM1_CURRENT_SAMPLE_TYPE		(CURRENT_SAMPLE_1SHUNT)


/*********************************************************************************/
// 当前MCU主频
#define MCU_MCLK_USED                   (MCU_MCLK_192M)
...
#define PLANTFORM_DRV_MODULE_NUM		(2)					// 驱动单元数目
#define PLANTFORM_MAX_CFG_PARA_ID		(2)					// 参数组数目
...
*
* 驱动单元模块个数为2时:
*		默认为驱动单元0 对应于 EPWM0和参数M0配置，驱动单元1 对应于 EPWM1和参数M1配置，
*		驱动单元可选择EPWM0或者EPWM1,并选择配置参数。
*		DU可选择DRVx_PWM_ID(EPWM0或者EPWM1),并选择配置参数DRVx_PARA_ID。
*			并需要更改DRVx_PARA_ID对应的MC_GlobalDef_Mx.h中的相应配置,
*			CURRENT_SAMPLE_TYPE_Mx 和 PWM_USED_ID_Mx 为 DRVx_CUR_SAMP_TYPE/DRVx_PWM_ID。
*
*
**********************************************************************************/
...
// 驱动单元1 下面定义用于消除编译错误，无实际意义，EPWM1_USED未启用
#define DRV1_PWM_ID						(1)
#define DRV1_CUR_SAMP_TYPE				(EPWM1_CURRENT_SAMPLE_TYPE)
#define DRV1_BEMF_CHECK_EABLE			(FUNCTION_OFF)		//	检查反电势采样通道数目和配置
...
#define EPWM0_USED						(FUNCTION_ON)		//   应用EPWM0
#define EPWM1_USED						(FUNCTION_ON)		//   应用EPWM0


//	多驱动单元配置
// 	default:		驱动单元DRV0 ------ EPWM0
//					驱动单元DRV1 ------ EPWM1

//	可进行选择配置，如  驱动单元DRV0 ------ EPWM1，但需要校验AD采样等配置是否满足


#define DRV0_PWM_ID						(PLANTFORM_EPWM0)	//  DRV0单元选用EPWM0
#define DRV0_PARA_ID					(0)					// 0 --- 参数组0 即M0参数配置
															// 1 --- 参数组1 即M1参数配置
#define DRV0_CUR_SAMP_TYPE				(EPWM0_CURRENT_SAMPLE_TYPE)
#define DRV0_BEMF_CHECK_EABLE			(FUNCTION_OFF)


#define DRV1_PWM_ID						(PLANTFORM_EPWM1)	//  DRV1单元选用EPWM1
#define DRV1_PARA_ID					(1)					// 0 --- 参数组0 即M0参数配置
															// 1 --- 参数组1 即M1参数配置
#define DRV1_CUR_SAMP_TYPE				(EPWM1_CURRENT_SAMPLE_TYPE)
#define DRV1_BEMF_CHECK_EABLE			(FUNCTION_OFF)		//	检查反电势采样通道数目和配置
...
**********************************************************************************/
	...
	#elif (DRV0_PARA_ID == 1)
		#define DRV0_CLOSE_LOOP				(CLOSE_LOOP_M1)
		#define DRV0_POWER_LIMIT_STATUS		(POWER_LIMIT_STATUS_M1)
		#define DRV0_SPPED_LIMIT_STATUS		(SPPED_LIMIT_STATUS_M1)
		#define DRV0_WEAKEN_FLUX_STATUS		(WEAKEN_FLUX_STATUS_M1)
		#define DRV0_PWM_ENABLE_STOP		(PWM_ENABLE_STOP_M1)
		#define DRV0_STOP_MODE				(STOP_MODE_M1)
		#define DRV0_UNDVOLT_PROT_ENABLE	(U_UNDVOLT_PROT_ENABLE_M1)
		#define DRV0_DIR_CHECK_ENABLE		(DIR_CHECK_STATUS_M1)
		#define DRV0_POS_SEEK_ENABLE		(SEEK_POSITION_STATUS_M1)
		#define DRV0_ALLIGN_CUR_ENABLE		(ALLIGN_STATUS_M1)
		#define DRV0_CHARGE_ENABLE			(CHARGE_STATUS_M1)
	#else
...
#define ADDR_PARA_LOCATION				(0x3FC00)
