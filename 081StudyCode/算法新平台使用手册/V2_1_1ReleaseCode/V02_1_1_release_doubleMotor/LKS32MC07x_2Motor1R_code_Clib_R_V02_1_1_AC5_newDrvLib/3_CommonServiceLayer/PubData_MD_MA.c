...
s16 getBemfTrackMinRecovVolt(u8 t_objId)
{
	PSTR_ControlObject pObj = getCtrlObjectPtr(t_objId);
	PSTR_DrvCfgPara pDrvCfgPara = pObj->m_pDrvCfgPara;

	return pDrvCfgPara->mS_FBDirCheck.nBemfTrackMinVoltReCov;
}
...
void updateSVPWMParaMALayer(PSTR_ControlObject pCtrlObj)
{
	...
	#if(DRV0_CUR_SAMP_TYPE == CURRENT_SAMPLE_1SHUNT)	
	{
		...
		t_wTmp32 = pCtrlObj->m_pDrvCfgPara->mS_GlobalCfg.m_nuDeadZone 
					+ pCtrlObj->m_pDrvCfgPara->mS_FBCurSense.nCurSampStableTime
					+ pCtrlObj->m_pDrvCfgPara->mS_FBCurSense.nPropagateTime;
		t_wTmp32 = (s64)t_wTmp32 * pCtrlObj->m_pDrvCfgPara->mS_GlobalCfg.m_nPWMClkFreq/1000;
	...
	#else
	{
		...
		t_wTmp32_1 = pCtrlObj->m_pDrvCfgPara->mS_FBCurSense.nPropagateTime;		
		t_wTmp32_1 = (s64)t_wTmp32_1 * pCtrlObj->m_pDrvCfgPara->mS_GlobalCfg.m_nPWMClkFreq/1000;

		// (Tdead + Tstable + Tadc) - Tadc + Tprop
		pSVPWMObjPar->nSampWaitTime3Shunt		= pSVPWMObjPar->nuSampStableTime3Shunt - (s16)t_wTmp32 + t_wTmp32_1;

		// (Tdead + Tstable + Tadc)/2 - Tadc - Tprd + Tprop
		t_wTmp32 = t_wTmp32_2 - (s16)t_wTmp32 - pSVPWMObjPar->nuPwmPrd + t_wTmp32_1;
...
void updateSVPWMParaMALayer(PSTR_ControlObject pCtrlObj)
{
	...
	if(pSVPWMObjPar->bSampType == CURRENT_SAMPLE_1SHUNT)	
	{
		pSVPWMObjPar->nuMinPwmSampTime3Shunt	= 0;
		pSVPWMObjPar->nuSampStableTime3Shunt	= 0;
	...
	else if (pSVPWMObjPar->bSampType == CURRENT_SAMPLE_2SHUNT)	
	{
		// 预留采样时间 Tzero = (Tdead + Tstable) + Tadc >= 2Tadc
		// 最大占空比为 Tprd - Tzero/2
		// 采样点为:    Tzero/2 - Tadc - Tprd   一定大于(-Tprd)
	...
	else if ( (pSVPWMObjPar->bSampType == CURRENT_SAMPLE_3SHUNT) 
			|| (pSVPWMObjPar->bSampType == CURRENT_SAMPLE_MOSFET)
			)
	{
