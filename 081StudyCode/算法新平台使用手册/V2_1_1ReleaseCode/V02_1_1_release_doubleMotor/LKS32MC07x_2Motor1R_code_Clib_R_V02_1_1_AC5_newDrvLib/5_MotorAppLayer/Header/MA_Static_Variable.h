...
typedef struct
{
	...
	#if (MA_OBJ_MAX_DEFINE > 1)							// 多电机时编译，单电机应用到功率计算时编译
	pSTR_PowerDef			m_pPower;					//  功率计算结构体
	#elif ((DRV0_CLOSE_LOOP == POWER_LOOP) || ((DRV0_POWER_LIMIT_STATUS == TRUE) && (DRV0_CLOSE_LOOP == SPEED_LOOP)))
	pSTR_PowerDef			m_pPower;					//  功率计算结构体
	#endif
...
typedef  struct
{
	...
	STR_MotorExceptionMA	m_MotorException;			//	直流电压异常检测结构体
	#elif(DRV0_UNDVOLT_PROT_ENABLE == 1)
