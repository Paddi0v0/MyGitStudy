...
static void parseUACmdAndSetpoint(STR_MACtrProc *pObj)
{
	...
	switch (pUa2Ma->eAppCmd)												// 解析UA层的控制命令
	{
		...
		case E_CMD_MOTOR_STOP:				// 使能MOE停止电机
		{
			pObj->m_SMACtrlBit.m_bUAMOE			= 1;
			pObj->m_SMACtrlBit.m_bMC_RunFlg		= 0;
			pObj->m_eStopMode					= pUa2Ma->eBrakeMode;				
			break;
		}

		case E_CMD_MOTOR_SPEED_RUN:			// 速度环运行电机，转动方向为设定方向
		{
			if(pObj->m_SMACtrlBit.m_bMC_RunFlg == 0)
			{
				pObj->m_nLoopMode				= SPEED_LOOP;		// 运行中不改变运行模式
			}			
			UARunCmd2MA(pObj, FALSE);								// 解析UA层发送给MA层的控制命令
			break;
		}

		case E_CMD_MOTOR_SPEED_RUN_REV:		// 速度环运行电机，转动方向为设定方向相反方向
		{
			if(pObj->m_SMACtrlBit.m_bMC_RunFlg == 0)				// 运行中不改变运行模式
			{
				pObj->m_nLoopMode				= SPEED_LOOP;
			}				
			UARunCmd2MA(pObj, TRUE);								// 解析UA层发送给MA层的控制命令
			break;
		}

		case E_CMD_MOTOR_TORQUE_RUN:		// 电流环运行电机，转动方向为设定方向
		{
			if(pObj->m_SMACtrlBit.m_bMC_RunFlg == 0)				// 运行中不改变运行模式
			{
				pObj->m_nLoopMode				= CURRENT_LOOP;					
		...
		case E_CMD_MOTOR_TORQUE_RUN_REV:	// 电流环运行电机，转动方向为设定方向相反方向
		{
			if(pObj->m_SMACtrlBit.m_bMC_RunFlg == 0)
			{
				pObj->m_nLoopMode				= CURRENT_LOOP;
			}
			UARunCmd2MA(pObj, TRUE);
			break;
		}

		case E_CMD_MOTOR_POWER_RUN:			// 功率环运行电机，转动方向为设定方向
		{
			if(pObj->m_SMACtrlBit.m_bMC_RunFlg == 0)				// 运行中不改变运行模式
			{
				pObj->m_nLoopMode				= POWER_LOOP;
		...
		case E_CMD_MOTOR_POWER_RUN_REV:		// 功率环运行电机，转动方向为设定方向相反方向
		{
			if(pObj->m_SMACtrlBit.m_bMC_RunFlg == 0)
			{
				pObj->m_nLoopMode				= POWER_LOOP;
	...
	if(pObj->m_nLoopMode == POWER_LOOP)
	{	//	功率环限制速度模式
		pObj->m_pAppCommData->bLoopLmtEnable	= pDrvCfgPara->mS_GlobalCfg.m_buSpdLmtInPowermode;
	}
}
    ...
    switch (pObj->m_eSysState)
   	{
			...
			if(StopMotorMain(pObj))				// 停止STOP状态任务函数，初始化完成时返回TRUE
			{
				...
				{	// 预充电使能时，进行预充电处理
					// 预充电未使能时
						...
						//        未使能  ----->  IPD判断   ------ 使能   运行IPD
						//                                  ------ 未使能 运行对相处理
					#if ( (PLANTFORM_DRV_MODULE_NUM>1) || ((PLANTFORM_DRV_MODULE_NUM==1) &&(DRV0_CHARGE_ENABLE==FUNCTION_ON)) )
					pObj->m_eNextSysState = E_DRIVER_CHARGE;
						...
						#elif ( (PLANTFORM_DRV_MODULE_NUM>1) || ((PLANTFORM_DRV_MODULE_NUM==1) &&(DRV0_POS_SEEK_ENABLE==FUNCTION_ON)) )
		...
		#if ( (PLANTFORM_DRV_MODULE_NUM>1) || ((PLANTFORM_DRV_MODULE_NUM==1) &&(DRV0_CHARGE_ENABLE==FUNCTION_ON)) )			
			/* 多电机时编入预充电函数，单电机使能预充电功能时才编入预充电状态*/
			...
			switch (GetMotorInfoSubStatus(pObj))
			{
				...
				case MOTOR_INFO_TRACKING:			//	顺逆风判断
				{
					...
					else
					{
						...
						//  		 					------ 未使能 运行对相处理
						#if ( (PLANTFORM_DRV_MODULE_NUM>1) || ((PLANTFORM_DRV_MODULE_NUM==1) &&(DRV0_POS_SEEK_ENABLE==FUNCTION_ON)) )					
						SetMotorInfoSubStatus(pObj, MOTOR_INFO_IPD);
						#else
						pObj->m_eNextSysState = E_DRIVER_READY_RUN;
						#endif
				...
				case MOTOR_INFO_IPD:			//多电机 初始位置检测状态IPD
				{
					if(IsNeedIPD(pObj))			//	需要IPD检测?
				{
						if(MotorInitIPDMain(pObj))	// IPD检测初始化函数
					{
							SetMotorInfoSubStatus(pObj, MOTOR_INFO_IPD_RESULT);
						}
					}
					else
					{
				...
				case MOTOR_INFO_IPD_RESULT:		//	IPD检测状态
				{
				...
				break;
				#elif ((PLANTFORM_DRV_MODULE_NUM==1) &&(DRV0_POS_SEEK_ENABLE==FUNCTION_ON))
		...
		{
			#if ( (PLANTFORM_DRV_MODULE_NUM>1) || ((PLANTFORM_DRV_MODULE_NUM==1) &&(DRV0_ALLIGN_CUR_ENABLE==FUNCTION_ON)) )	
