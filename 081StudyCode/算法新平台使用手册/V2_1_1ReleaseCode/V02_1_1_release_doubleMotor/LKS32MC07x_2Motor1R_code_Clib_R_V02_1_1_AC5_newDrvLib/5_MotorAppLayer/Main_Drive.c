...
#if ((MA_OBJ_MAX_DEFINE>1) || ((MA_OBJ_MAX_DEFINE==1)&&(DRV0_CLOSE_LOOP==SPEED_LOOP)))
 ...
 *******************************************************************************/
	...
	#endif
#else
static void handleCurSetpointClr(STR_MACtrProc *pObj)			//	多电机
{
	pObj->m_pMotorSpeed->mSpeedPI.wIntegral = 0;				//	速度积分清0
	pObj->m_nQCurrentSet					= 0;				//	Iq设定值清0
	pObj->m_nQCur_Reference					= 0;				//	Iq滤波值清0
 ...
 *******************************************************************************/
	...
	#endif
#else
static void handleCurSetpoint(STR_MACtrProc *pObj,s16 temp16)	//	多电机
{
	pObj->m_pMotorSpeed->mSpeedPI.wIntegral = (s64)temp16 << 22;	//	速度积分设定
	pObj->m_nQCurrentSet					= temp16;				//	Iq设定值设定
	pObj->m_nQCur_Reference					= temp16;				//	Iq滤波值设定
...
/*******************************************************************************
 ...
 2023/2/19      V1.0           Tonghua Li          创建
 *******************************************************************************/
#if( (MA_OBJ_MAX_DEFINE>1) || ( (MA_OBJ_MAX_DEFINE==1)&&(DRV0_CLOSE_LOOP!=POWER_LOOP)) )
...
{	
	...
	if(pDrvCfgPara->mS_GlobalCfg.m_buStopPWMEnable == TRUE)
	{
		SetPWMCmdMA2MD(&pObj->mMaToMd, E_PWM_ON);		//	设置MOE使能操作命令到交换区，由MD层调用HD驱动函数实现MOE操作
	}
	else
	{
		SetPWMCmdMA2MD(&pObj->mMaToMd, E_PWM_OFF);		//	设置操作命令到交换区，由MD层调用HD驱动函数实现MOE操作
	}
...
void MotorTrakingMain(STR_MACtrProc *pObj)
{
	...
	if(pObj->mg_pCtrlObj->m_pDrvCfgPara->mS_GlobalCfg.m_buBemfDirEnable)	//	反电势电路顺逆风
	{
		SetPWMCmdMA2MD(pS_MA2MD, E_PWM_OFF);				//	传递控制命令到MD关闭MOE
		SetMotorCmdMA2MD(pS_MA2MD,E_MOTOR_BEMF_TRACKING);	//	设置MD层状态为反电势电路顺逆风检测状态
	}
	else				//	观测器顺逆风
	{
	...
	#else	
		#if ((MAX_DRV_MODULE_USED==1) &&(DRV0_BEMF_CHECK_EABLE==FUNCTION_ON))	//单电机，使能反电势电路顺逆风时编入代码
...
{	
	...
	#if( (MA_OBJ_MAX_DEFINE>1) || ( (MA_OBJ_MAX_DEFINE==1)&&(DRV0_UNDVOLT_PROT_ENABLE==1)) )
														//	电压异常保护是否启用
	s16 temp16;
...
void MotorBreakMain(STR_MACtrProc *pObj)
{
	...
	// update MA infor to MD
	if(pObj->m_eStopMode == E_STOP_ZERO_CUR)		//零电流
	{
		StateRun(pObj);												//	处理函数
		handleSpdSetpoint(pObj);									//	速度命令值处理
		if(pObj->mg_pCtrlObj->m_pDrvCfgPara->mS_FBSlvcCfg0.bVoltageProtectionEnable == 1)	//	电压异常保护
		{
			pObj->m_pMotorException->nBusVoltage	= pObj->mMdToMa.nVdc;
			if(motorStopOrExceptionHandle(pObj->m_SMACtrlBit.m_bMC_RunFlg, pObj->m_pMotorException,&temp16))
			{				
				handleCurSetpoint(pObj,temp16);						//	速度命令值处理
				pObj->m_SMACtrlBit.m_bExceptionEnable	= 1;		//	异常标志 置1   ---- 存在异常发生
			}
			else
			{				
				handleCurSetpointClr(pObj);							//	速度命令值处理
				pObj->m_SMACtrlBit.m_bExceptionEnable	= 0;		//	异常标志 清0   ---- 没有异常发生
			}
		}
		else
		{			
	...
	else if(pObj->m_eStopMode == E_STOP_SHORT)		//短接制动
	{
		StateBrake(pObj);											//	短接制动处理函数
		handleCurSetpointClr(pObj);									//	速度命令值处理
		if(pObj->mg_pCtrlObj->m_pDrvCfgPara->mS_FBSlvcCfg0.bVoltageProtectionEnable == 1)	//	电压异常保护使能 ?
		{
			if(motorStopOrExceptionHandle(pObj->m_SMACtrlBit.m_bMC_RunFlg, pObj->m_pMotorException,&temp16))
			{
				pObj->m_SMACtrlBit.m_bExceptionEnable	= 1;		//	异常标志 置1   ---- 存在异常发生
				SetPWMBrakeDuty(pObj,2048);							//	设置制动上桥臂PWM宽度 50% 	2048/4096
			}
			else
			{			
				pObj->m_SMACtrlBit.m_bExceptionEnable	= 0;		//	异常标志 清0   ---- 没有异常发生
				SetPWMBrakeDuty(pObj,0);							//	设置制动PWM上桥臂脉宽 0%
			}
		}
		else
		{			
		...
		SetPWMCmdMA2MD(pS_MA2MD, E_PWM_ON);							//	传递控制命令到MD使能MOE	
		SetCurRefMA2MD(pS_MA2MD,0,0);								//	设置dq轴电流指令值 0
		SetSpeedRefMA2MD(pS_MA2MD,pObj->m_pMotorSpeed->wSpeedRef);	//	传递速度指令值到MD层
		SetBrakePWMDutyMA2MD(pS_MA2MD,getPWMBrakeDuty(pObj));		//	传递PWM宽度到MD层
		SetMotorCmdMA2MD(pS_MA2MD,E_MOTOR_BRAKE);					//	设置MD层状态为运行状态 E_MOTOR_BRAKE
	}
	else				//	默认为 MOE关闭方式
	{
		...
		SetPWMCmdMA2MD(pS_MA2MD, E_PWM_OFF);						//	传递控制命令到MD关闭MOE	
		SetCurRefMA2MD(pS_MA2MD,0,0);								//	设置dq轴电流指令值 0
		SetSpeedRefMA2MD(pS_MA2MD,0);								//	传递速度指令0值到MD层
		SetMotorCmdMA2MD(pS_MA2MD,E_MOTOR_RESET);					//	设置MD层状态为运行状态 E_MOTOR_RESET
	}
...
static void StateInit(STR_MACtrProc *pObj)
{
    ...
    pObj->m_pOnTheFlyDetect->nMotorStopCurrentThd	= App2CoreCurTrans(pAppToCore,pDrvCfgPara->mS_FBDirCheck.m_wMotorStopCurrentThd);//MOTOR_STOP_CUR_THD_M0;    /*电机停止检测电流阈值 */
    pObj->m_pOnTheFlyDetect->nMotorStopCurDifThd	= App2CoreCurTrans(pAppToCore,pDrvCfgPara->mS_FBDirCheck.m_wMotorStopCurDifThd);//MOTOR_STOP_CUR_DIF_THD_M0; /*电机停止检测电流阈值 */
    pObj->m_pOnTheFlyDetect->wFreqAvg				= 0;
    pObj->m_pOnTheFlyDetect->nMotorVolMagAvg		= 0;
    ...
    /*****定位参数初始化********/
	t_wTmp = App2CoreCurTrans(pAppToCore,pDrvCfgPara->mS_FBAllign.m_wAlignCurAcc);
	pObj->m_pSTR_CurRampGen->wRampIncrease = ((s64)t_wTmp * 16777) >> 8;
	
	t_wTmp = App2CoreCurTrans(pAppToCore,pDrvCfgPara->mS_FBAllign.m_wAlignCurDec);
	pObj->m_pSTR_CurRampGen->wRampDecrease = ((s64)t_wTmp * 16777) >> 8;
	...
	pObj->m_pMotorSpeed->wSpeedRampACCStep	= App2CoreFreqTrans(pAppToCore,pDrvCfgPara->mS_FBSpdLoop.m_wSpdRunAcc)*65536/1000; //SPEED_RUN_ACC_M0		  
	pObj->m_pMotorSpeed->wSpeedRampDECStep	= App2CoreFreqTrans(pAppToCore,pDrvCfgPara->mS_FBSpdLoop.m_wSpdRunDec)*65536/1000; //SPEED_RUN_DEC_M0

	ramp32GenInit(&pObj->m_pMotorSpeed->STR_SpdRampGen);

	ramp32GenInit(pObj->m_pSTR_CurRampGen);

	pObj->m_pMotorSpeed->nSpeedFiltTime	= (s16)(10240/(10 + (s16)pDrvCfgPara->mS_FBSlvcCfg0.m_wSpeedFilTime));

	pObj->m_wPower001WFil				= 0;
	pObj->m_nPower001WFactor			= 1024/(1+10);
	/*****速度环参数初始化********/
	if (pObj->m_nLoopMode == SPEED_LOOP)
    {
        pObj->m_pMotorSpeed->mSpeedPI.KP = pDrvCfgPara->mS_FBSpdLoop.PASRKp;   //速度环 Kp	P_ASR_KP_M0
        pObj->m_pMotorSpeed->mSpeedPI.KI = pDrvCfgPara->mS_FBSpdLoop.PASRKi;   //速度环 Ki P_ASR_KI_M0
        pObj->m_pMotorSpeed->mSpeedPI.wIntegral = 0;
		...
		s32 t_wTmpKp;
		s32 t_wTmpKi;
		t_nTmp = (s16)pDrvCfgPara->mS_FBSlvcCfg0.m_wSpeedFilTime + 25;		
		SpeedPIAutoTunning(pObj->mg_nMACtrlProcID,
						5000,
						t_nTmp,
						&t_wTmpKp,
		...
		pObj->m_pMotorSpeed->mSpeedPI.KP = sat(t_wTmpKp,0,32767);
		pObj->m_pMotorSpeed->mSpeedPI.KI = sat(t_wTmpKi,0,32767);
		#endif
        ...
        pObj->m_pMotorSpeed->wSpeedfbk				= 0;
		pObj->m_pMotorSpeed->wSpeedfbkFilter		= 0;
        pObj->m_pMotorSpeed->wPowerLimitSpeedChangeSet	= 0;
		pObj->m_pMotorSpeed->wPowerLimitSpeedChangeSetInc	= 0;
    ...
    /*****功率环参数初始化********/
	else if (pObj->m_nLoopMode == POWER_LOOP)
    {
        PowerLoopInit(pObj);
	...
	t_nTmp	= (s16)App2CoreCurTrans(pAppToCore,pDrvCfgPara->mS_FBSlvcCfg0.m_wIdStartSet);
	
	t_wTmp	= App2CoreCurTrans(pAppToCore,pDrvCfgPara->mS_FBSlvcCfg0.m_wIqStartSet);
	t_nuTmp	= pDrvCfgPara->mS_FBSlvcCfg0.TransTime2Close;

	InitDQCurRefGenWithWeakenFlux(pObj->mg_nMACtrlProcID,t_nTmp,(s16)t_wTmp, (s16)t_nuTmp);
	

    /*****对相环路计算时间参数初始化********/
    pObj->m_pTime->nChargeTime			= 0;
	pObj->m_pTime->nChargePeriod		= pDrvCfgPara->mS_GlobalCfg.m_nuChargeTime;		 //CHARGE_TIME_M0
	
    pObj->m_pTime->nAlignTime			= 0;
	pObj->m_pTime->nAlignLowPeriod		= pDrvCfgPara->mS_FBAllign.nPlat1Time;			 //DC_HOLDTIME_TIME_LENTH_M0
	pObj->m_pTime->nAlignAllPeriod		= pDrvCfgPara->mS_FBAllign.nPlat1Time + pDrvCfgPara->mS_FBAllign.nPlat2Time; //DC_ALIGN_TOTAL_LENTH_M0
	
    pObj->m_pTime->nDirCheckTime		= 0;
	pObj->m_pTime->nDirCheckPeriod		= pDrvCfgPara->mS_FBDirCheck.nSpdTrackDelayTime; //SPEED_TRACK_DELAYTIME_M0
	
    pObj->m_pTime->nStopDelayTime		= 0;
	pObj->m_pTime->nStopDelayTimePeriod	= pDrvCfgPara->mS_FBDirCheck.nMotorStopDelayTime; //STOP_DELAY_TIME_M0
	
    pObj->m_pTime->nStopDelayCntr		= 0;
	pObj->m_pTime->nStopDelayPeriod		= pDrvCfgPara->mS_FBDirCheck.nMotorStopTime;	//STOP_TIME_M0
...
{	
	...
	else
	{
		...
		else if( pObj->m_pTime->nChargeTime ==(pObj->m_pTime->nChargePeriod * 3))	// CHARGE_TIME为每段预充电时间，可根据硬件实际设置
		{
			pObj->m_pTime->nChargeTime ++ ;
			SetPWMCmdMA2MD(&pObj->mMaToMd,E_PWM_OFF);
		}
	...
	if(pObj->m_pAppCommData->bChargeEndFlag == 2)//预充电完成
	{
		if(pObj->mMdToMa.nuFineStatus == E_FC_NON)
		{
			...
			pObj->m_pAppCommData->bChargeFlag			= 2;
		}
		SetPWMCmdMA2MD(&pObj->mMaToMd,E_PWM_OFF);
	}
...
//	s32 t_wTemp;	
	...
	if(pObj->m_nLoopMode == SPEED_LOOP)
	{
		if(++pObj->m_pAppCommData->nLoopCntr >= pObj->m_pAppCommData->nLoopPeriod)    //速度环环路计算周期。
		{
			...
			pObj->m_pMotorSpeed->wSpeedSet = App2CoreFreqTrans(pAppToCore,pObj->mUaToMa.wFreqCmd); //速度参考给定

			if(pObj->m_pAppCommData->bLoopLmtEnable)		//限功率使能，速度环限制最大输出功率
			{
				pObj->m_pPower->wPowerValue				= pObj->m_wPower001W;
				pObj->m_pMotorSpeed->wPowerLimitValue = pObj->mUaToMa.wPowerCmd;
				PowerLimitCalc(pObj->m_pMotorSpeed,pObj->m_pPower->wPowerValue);	//	功率限制， 输出频率改变值wPowerLimitSpeedChangeSet
																					//	调整频率设定值
				pObj->m_pMotorSpeed->wSpeedSet = pObj->m_pMotorSpeed->wSpeedSet - pObj->m_pMotorSpeed->wPowerLimitSpeedChangeSet;
			}			

			HandleSpdSetpointCmd(pObj);			
			HandleSpdSetpointRamp(pObj);
			pObj->m_pMotorSpeed->wSpeedRef	= pObj->m_pMotorSpeed->STR_SpdRampGen.wRampOut;
			
			QCurRefGenValue(pObj->mg_nMACtrlProcID,&pObj->m_pMotorSpeed->mSpeedPI.wIntegral,22,pObj->m_pMotorSpeed->wSpeedSet);

			if(pObj->m_pAppCommData->bOpenRunFlag == 1)
			{
				pObj->m_pMotorSpeed->mSpeedPI.wIntegral = 0;
				pObj->m_pAppCommData->bOpenRunFlag = 2;
			}
			...
			pObj->m_nQCurrentSet = SpeedLoopReg(pObj->m_pMotorSpeed);// 速度环PI计算

		}
	}

	/*******************电流环处理*******************************/		
	else if (pObj->m_nLoopMode == CURRENT_LOOP)
	{
		PSTR_DrvCfgPara	pDrvCfgPara = pObj->mg_pCtrlObj->m_pDrvCfgPara;		
		s32 t_wTemp1,t_wTemp;
		...
		pObj->m_pMotorSpeed->wSpeedSet = App2CoreFreqTrans(pAppToCore,t_wTemp1); //速度参考给定
		
		HandleSpdSetpointCmd(pObj);							//	电流环下 频率指令处理
		
		if(ABS(pObj->m_pMotorSpeed->wSpeedfbk) >= ABS(pObj->m_pMotorSpeed->wSpeedSet))
		{
			pObj->m_pMotorSpeed->wSpeedSet	= pObj->m_pMotorSpeed->wSpeedfbk;
		}	
	...
	else if(pObj->m_nLoopMode == POWER_LOOP)
	{
		...
		if(++pObj->m_pAppCommData->nLoopCntr >= pObj->m_pAppCommData->nLoopPeriod) //功率环环路计算周期。
		{
			...
			pObj->m_pPower->wPowerSet			= pObj->mUaToMa.wPowerCmd;				// 0.01W
			pObj->m_pPower->wPowerValue			= pObj->m_wPower001W;
			if(pObj->m_pAppCommData->bLoopLmtEnable)     //限速功能使能 ，功率环限制最大转速。
			{
				//读取速度限制值
				pObj->m_pPower->wSpeedLimitValue	= App2CoreFreqTrans(pAppToCore,pObj->mUaToMa.wFreqCmd);
				SpeedLimitCalc(pObj->m_pMotorSpeed->wSpeedfbk, pObj->m_pPower);	//限速度,限制功率输出计算
		...
		if(pObj->m_SMACtrlBit.m_bMotorDirtion == CW)							//	功率环下速度指令处理
		{
			if(pObj->m_pMotorSpeed->wSpeedSet < 0 )
			{
				pObj->m_pMotorSpeed->wSpeedSet = -pObj->m_pMotorSpeed->wSpeedSet;
			}					
		}
	...
	pObj->m_nDCurrentSet = getDCurRefGenWithEnWeakenFlux(pObj->mg_nMACtrlProcID);	//	d轴电流指令处理
