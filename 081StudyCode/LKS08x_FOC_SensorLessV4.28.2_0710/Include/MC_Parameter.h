...
#define AMPLIFICATION_GAIN             (3.984)            /* 运放放大倍数 */  
...
#define U_RATED_CURRENT                (2.16)              /* 单位:A, 电机额定工作电流，相电流最大值 */  // 这里限电流乘根号3
#define U_MAX_FREQ                     (500)            /* 单位:Hz,  电机最高运行转速 */ // N极对数=60F电转速/S机械速度 在这里要乘2

/* ------------------------------Motor Parameter------------------------------ */
#define U_MOTOR_PP                     (5.0)              /* 电机极对数 */
#define U_MOTOR_RS                     (2.45)         /* 单位: Ω 电机相电阻 */ //测出来的是2项电阻 填写除2
#define U_MOTOR_LD                     (3425)          /* 单位: uH 电机d轴电感 */ // 100uh  150uh
#define U_MOTOR_LQ                     (3925)          /* 单位: uH 电机q轴电感 */
...
#define U_START_CUR_SET_F              (0.9)              /* 单位: A 第一段定位电流 */
#define U_START_CUR_SET_S              (1.8)              /* 单位: A 第二段定位电流 */
...
#define ALIGN_CURRENT_ACC              (0.10)              /* 单位: (1/8)A/ms  定位电流加速调整值  初始位置检测使能后给到最大值，不能超过30，否则数据会溢出。 */
#define ALIGN_CURRENT_DEC              (0.10)              /* 单位: (1/8)A/ms  定位电流减速调整值  初始位置检测使能后给到最大值，不能超过30，否则数据会溢出。 */

/*---------------------------------开环参数------------------------------------*/
#define OPEN_ANGLE_TAG_FREQ            (25.0)             /* 单位：Hz 开环拖动最终频率 */
...
/*---------------------- -----开环闭环切换过渡参数------------------------------*/
#define OPEN2CLOSE_RUN_COV_TIME        (35)               /* 开环闭环切换过渡时间：单位：mS */
#define OPEN2CLOSE_RUN_CURRENT_RAMP    (0.07)             /* 开环闭环切换过渡内，D,Q轴电流变化斜率。单位：A/ms */
...
/* ------------------------------观测器PLL PI参数----------------------------- */
#define PLL_KP_GAIN                    70                /* PLL_Kp 估算器Kp */
...
#define P_CURRENT_KI                   (900)             /* 电流环Ki，实际运用的Kp会根据这个值和电机参数计算出最终的Ki */ // 默认值1000

#define AUTO_FW_LIM                    ((s16)0)            /* 自动弱磁D轴电流限制，Q12格式，最大值 4096 */

#define TORQUE_MODE_CURRENT_CHANGE_ACC (0.1)  //0.08           /* 单位：A/ms 电流加速调整值 */
#define TORQUE_MODE_CURRENT_CHANGE_DEC (0.1)  //0.08             /* 单位：A/ms 电流减速调整值 */
...
#define SPEED_SET                      (85)              /* 单位：Hz  速度给定值 */
#define SPEED_LOOP_CNTR                (4)                /* 单位：ms  速度环路计算周期 */
...
#define IQMAX                          (2.0)              /* 单位:A, 速度环输出最大值 */
#define IQMIN                          (-2.0)             /* 单位:A, 速度环输出最小值 */

#define SPEED_RUN_ACC                  (0.1)    //0.1          /* 单位 (1/128)Hz 速度加速调整值 */
#define SPEED_RUN_DEC                  (0.1)      //0.1          /* 单位 (1/128)Hz 速度减速调整值 */
...
#define U_DCB_OVERVOLTAGE_RECOVER      (21.5)               /* 单位：V 过压恢复设定值 */
