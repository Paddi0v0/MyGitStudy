...
void FOC_Model(stru_FOC_CtrProcDef *pCtrProc)
{
    ...
    switch(pCtrProc->eSysState)
    {
	  case BEMF_CHECK:     /*反电动势检测*/
     {
        ...
        {
					 #if (CURRENT_SAMPLE_TYPE == CURRENT_SAMPLE_2SHUNT)
             Bemf_CheckProcess(&stru_BEMF_Check, struBemfVolt.nPhaseW, struBemfVolt.nPhaseV, struBemfVolt.nPhaseU);    //C相 B相 A相
					 #else
					 #if (CURRENT_SAMPLE_TYPE == CURRENT_SAMPLE_1SHUNT)
					   Bemf_CheckProcess(&stru_BEMF_Check, struBemfVolt.nPhaseW, struBemfVolt.nPhaseV, struBemfVolt.nPhaseU);    //C相 B相 A相
					 #endif
            ...
            BemfSwitch(&stru_BEMF_Check);   //切换时候的赋值
        }
        break;
			}
    case DIR_CHECK:
    ...
    {   /* 直流制动 */
		...
		case BEMF_BRAKE:
        {
            ...
            this->mVoltUVW_NegPWM.nPhaseW = 0;
            #endif
					  this->mVoltUVW_PWM.nPhaseU = 0;  /* 下桥全开 刹车 */
...
void CurrentLoopReg(stru_FOC_CurrLoopDef *this)
{
    ...
    this->mStatVoltDQ.nAxisD = 1000;     /* D轴电流PI计算， 输出Vd 4096 对应额定电压*/
#else
