...
void StateInit(void)
{
    ...
    struAppCommData.wDirTrackFreq  = User2AppFreqTrans(BEMF_TRACK_ON_FREQ_THH);  //电机大于此频率 切入闭环频率
...
/*****************************************************************************
 * 函数名   : void StateBEMFDirCheck(void)
 * 说明     : 顺逆风检测程序，通过反电势检测速度值
...
void StateBemfDirCheck(void)
{
    ...
    if(++struTime.nBemfheckTime >= BEMF_CHECK_TIME)
    {
        ...
        case CW://顺时针
        {
            if(stru_BEMF_Check.nBemfUVWAmpValue > struAppCommData.nBemfTrackVolt)     //单位：0.01V
            {
                if(stru_BEMF_Check.nBemf_FreqAve < (-struAppCommData.wBemfTrackFreq))     /*反方向转速太高，则等待降速后再处理，否则会造成过流、过压*/
                {
                    struFOC_CtrProc.eSysState = BEMF_BRAKE;
                }
                else if(stru_BEMF_Check.nBemf_FreqAve < struAppCommData.wDirTrackFreq)    //低速
                {
                    struFOC_CtrProc.eSysState = BEMF_BRAKE;
                ...
                {
                    struFluxOB_Param.nSTO_KP = PLL_KP_GAIN;              /* 运行观测器PLL KP 参数 */
                    struFluxOB_Param.nSTO_KI = PLL_KI_GAIN;              /* 运行观测器PLL KI 参数 */
        ...
        case CCW://逆时针
        {
            if(stru_BEMF_Check.nBemfUVWAmpValue > struAppCommData.nBemfTrackVolt)//struAppCommData.nBemfTrackVolt单位0.01V
            {
                if(stru_BEMF_Check.nBemf_FreqAve > (struAppCommData.wBemfTrackFreq))  /* 正方向转速太高，则等待降速后再处理，否则会造成过流、过压*/
                {
                    struFOC_CtrProc.eSysState = BEMF_BRAKE;
                }
                else if(stru_BEMF_Check.nBemf_FreqAve > (-struAppCommData.wDirTrackFreq))//低速
