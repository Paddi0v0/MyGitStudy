...
void Hardware_init(void)
{
    ...
    NVIC_SetPriority(UART1_IRQn, 0); //UART1中断优先级配置
    NVIC_EnableIRQ(UART1_IRQn);      //使能UART1定时器中断
 #if POWER_MODE	
...
void UART_init(void)
{
    ...
    UART_InitStruct.FirstSend = UART_FIRSTSEND_MSB;                      /* 先发送MSB 高端先发送*/
    UART_InitStruct.ParityMode = UART_Parity_EVEN;                         /* 偶校验 */
