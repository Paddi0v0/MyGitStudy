...
void ADC0_init(void)
{
  ...
  ADC_InitStructure.MCPWM_Trigger_En = ADC_MCPWM_T0_TRG | ADC_MCPWM_T1_TRG; //MCPWM_T0和T1触发ADC采样
  ADC_InitStructure.FirSeg_Ch = ADC_2_TIMES_SAMPLE;                         //第一段采样的总通道数: 1~20
  ...
  ADC0_IF = 0xff;   //清中断标志位
  ADC0_GAIN0 = 0x0; //开启2/3增益
  ADC0_CHN0 = ADC_CHANNEL_9 | (ADC_CHANNEL_18 << 8);
  ADC0_CHN1 = ADC_CHANNEL_9 | (ADC_CHANNEL_18 << 8);
}
#endif
#if ADC_MODE4
void ADC0_init(void)
