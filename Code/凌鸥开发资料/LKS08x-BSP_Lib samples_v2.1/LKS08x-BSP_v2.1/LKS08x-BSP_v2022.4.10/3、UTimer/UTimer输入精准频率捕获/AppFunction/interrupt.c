...
void UTIMER0_IRQHandler(void)
{
	...
	if (UTIMER_IF & BIT2) //判断UTimer0的CH1是否发生捕获上升沿中断 TIM0_CH1: P0.14
	{
		...
		else //捕获第二个上升沿
		{
			...
			{
				//(96000000/(InPut_CurrentTime - InPut_FrontTime))+1;                    //单位HZ  原始计算，便于理解
				InPut_PulseFreq = (InPut_CurrentTime - InPut_FrontTime); //脉冲周期计数值
			}
			else //捕获时间有溢出
			{
				// InPut_PulseFreq = 1/((OverFlow - 1)*0.0005 + ((48000 - InPut_FrontTime)+ InPut_CurrentTime)*0.0005/48000);  //单位HZ  原始计算，便于理解
				InPut_PulseFreq = (480 * OverFlow - InPut_FrontTime + InPut_CurrentTime); //脉冲周期计数值
			}
		}
	}
}
