...
void Hardware_init(void)
{
    ...
    GPIO_init();             /* GPIO初始化 */
    EXTI_init();             /*外部中断初始化*/
	  NVIC_EnableIRQ(GPIO_IRQn);	 //使能GPIO外部中断
  	NVIC_SetPriority(GPIO_IRQn, 1);//GPIO外部中断优先级设置为1
    __enable_irq();          /* 开启总中断 */
}
...
void EXTI_init(void)
{
	  ...
	  EXTI_InitStruct.EXTI_Pad = EXTI_Pad_0;           //开启P0.0外部触发
	  EXTI_InitStruct.EXTI_Trig =  EXTI_TRG_Rising;    //上升沿触发
