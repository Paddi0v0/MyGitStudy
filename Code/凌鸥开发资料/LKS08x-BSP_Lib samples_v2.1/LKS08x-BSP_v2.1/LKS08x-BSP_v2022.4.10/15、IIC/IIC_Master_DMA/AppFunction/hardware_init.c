...
void IIC_init(u16 div0)
{
    ...
    I2C_InitStruct.ADRCMP = DISABLE;   //I2C地址按硬件比较，1:启用，0:禁用
    I2C_InitStruct.ADDR = 0x00;        //I2C地址，7位
    I2C_InitStruct.MASTER = ENABLE;    //I2C主模式，1:启用，0:禁用
    I2C_InitStruct.SLAVE = DISABLE;    //I2C从模式，1:启用，0:禁用
    ...
    I2C_InitStruct.STOPINT = DISABLE;   //I2C停止事件（STOP）中断启用，1:启用，0:禁用
...
void i2c_tx(u8 addr, u8 *data, u8 len)
{
    ...
    I2C0_ADDR = addr & 0x7f;                          // 从设备地址
    I2C0_BCR = (I2C->BCR & 0xf0) | ((len - 1) & 0xf); // 设定数据长度
    I2C0_SCR |= BIT2;                                 // I2C传输方向
