...
#define UART_IF_FLG            UART0_IF

#define USART_TX_EN_FLG()      uart_tx_en
#define USART_ReceiveData()    UART0_BUFF
void UART0_SENDDATA(unsigned char n);
#define USART_SendData         UART0_SENDDATA
...
* SCI access macros
*****************************************************************************************/
#define TXD_IE_DISABLE()       {UART0_IE &= (~0x05);}
#define TXD_IE_ENABLE()        {UART0_IE |= 0x05;}
#define RXD_IE_DISABLE()       {}
#define RXD_IE_ENABLE()        {UART0_IE |= 0x02;}

#define CLEAR_RXNE()           {UART0_IF |= USART_FLAG_RXNE;}
#define CLEAR_TX_EMPT()        {UART0_IF |= USART_FLAG_TX_EMPT;}
#define CLEAR_TX_TC()          {UART0_IF |= USART_FLAG_TC;}
